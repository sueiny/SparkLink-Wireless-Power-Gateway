...
`sle_data_app` 的上行数据链路已经进入真实 SLE 联调阶段；默认启动为 `real`，Mock 需要显式 `--mode mock`。下行命令链路现在按高内聚边界收敛为 raw ST bridge：`gatewayd` 负责业务命令、Modbus RTU 和 ST DATA 封装，`sle_data_app` 只负责 SLE 连接管理和 raw ST 写转发。

因此当前验收要分开看：

- 上行真实数据：可以用 `COM23 + COM36` 两路 Root 压测全拓扑是否上云。
- 下行命令通道：可以验证 ThingsKit/离线规则 -> gatewayd -> raw ST IPC -> `sle_data_app` -> SLE write 是否闭环。
- 下行真实执行：必须结合 DTU COM 打印校验 ST 帧、Modbus 功能码和 CRC，不能只看 MQTT response。
...
| 命令处理 | `sle_data_app/src/sle_cmd_handler.c` | 只处理 `CMD_METHOD_RAW_ST_DOWNLINK`，校验 ST 头后转给 SLE write。 |
...
ThingsKit RPC request
  ...
  -> CommandValidator
  -> CommandExecutor
  -> gatewayd Modbus + ST DATA builder
  -> IpcCmdSender
  -> ipc_cmd_receiver
...
| `method` | 1 | `CMD_METHOD_RAW_ST_DOWNLINK=100` 或保留方法枚举。 |
| `param_len` | 2 | 参数长度，小端。 |
| `param_data` | `param_len` | raw ST 下行参数或保留参数。 |

`CMD_METHOD_RAW_ST_DOWNLINK` 的 `param_data`：

| 字段 | 长度 | 含义 |
| --- | --- | --- |
| `root_id` | 2 | 目标 root 节点 ID，小端，用于选择 SLE 连接。 |
| `dst_node_id` | 2 | ST 帧内传输目标节点 ID，小端；下行固定为目标 Root。 |
| `st_frame_len` | 2 | 完整 ST 帧长度。 |
| `st_frame` | `st_frame_len` | gatewayd 已封装好的完整 ST DATA 帧。 |
...
- `gatewayd` 在 `CommandExecutor::executeViaIpc()` 里拒绝超过 `IPC_CMD_MAX_PARAM_LEN` 的 raw ST 参数。
- `sle_data_app` 在 `ipc_cmd_receiver.c` 里校验 `param_len > IPC_CMD_MAX_PARAM_LEN` 并拒绝处理。
- `sle_cmd_handler.c` 会校验 ST magic/version、Root 传输 `dst_node_id` 一致性和长度边界。

## 支持命令对照

| 目标 | method/service | gatewayd 行为 | sle_data_app 当前行为 | 真实执行状态 |
| --- | --- | --- | --- | --- |
| `METER_*` | `set_relay` | 根据配置构造 meter Modbus `0x10` 与完整 ST DATA，ST `dst_node_id` 指向 Root | raw ST 写入 READY root 的 SLE data property | 已接入真实 SLE write；以 root COM 完整包解析为准。 |
| `RELAY_*` | `set_relay` | 根据配置构造 relay Modbus `0x05` 与完整 ST DATA，ST `dst_node_id` 指向 Root | raw ST 写入 READY root 的 SLE data property | 已接入真实 SLE write；以 root COM 完整包解析为准。 |
...
| `dtu_node` | `reboot` | 当前继续走 gatewayd 模拟/预留路径 | 不进入 raw ST handler | 真实 root reboot 待实现。 |
| 外接设备/DTU | `set_mode`、`set_collect_cycle`、`trigger_collect` | 当前继续走 gatewayd 模拟/预留路径 | 不进入 raw ST handler | 后续需要 gatewayd 先定义对应 ST/Modbus 帧。 |
...
10. `sle_data_app/src/ipc_cmd_receiver.c` 与 `sle_cmd_handler.c`：确认 raw ST 参数校验和转发。

## 后续真实下行落地方向

后续真实下行扩展不应把业务解析放回 `sle_data_app`。新增命令时先在 `gatewayd` 中定义物模型、Modbus/ST 构造和状态补丁，再通过 `CMD_METHOD_RAW_ST_DOWNLINK` 发送完整 ST 帧。

多 root 路由：`sle_data_app` 会从 root 自身上行 ST 帧学习 `root_id -> server_index` 映射。已学习到映射时按 `root_id` 精确转发；未学习且只有一个 READY root 时允许 fallback；未学习且多个 root 同时 READY 时拒绝转发，避免错发。
