...
| `dst_node_id` | 2 | ST 帧内传输目标节点 ID，小端；下行指向目标 DTU。 |
...
| --- | --- | --- | --- | --- |
| `METER_*` | `set_relay` | 根据配置构造 meter Modbus `0x10` 与完整 ST DATA，ST `dst_node_id` 指向目标 DTU | raw ST 写入 READY root 的 SLE data property | 已接入真实 SLE write；以 root COM 完整包解析为准。 |
| `RELAY_*` | `set_relay` | 根据配置构造 relay Modbus `0x05` 与完整 ST DATA，ST `dst_node_id` 指向目标 DTU | raw ST 写入 READY root 的 SLE data property | 已接入真实 SLE write；以 root COM 完整包解析为准。 |
