/**
 * @file dtu_storage.c
 * @brief DTU runtime 配置唯一拥有者，负责默认值、NV 加载、提交和恢复出厂。
 */
...
/* NV 临时缓存放在静态区，避免 DtuInitTask/协议任务栈承载较大结构体。 */
...
/* 固定 MAC 支持 AABBCCDDEEFF 和 AA:BB:CC:DD:EE:FF 两种格式。 */
...
errcode_t dtu_storage_load(void)
{
    ...
    dtu_storage_set_default(dtu_storage_runtime()); // 先铺默认值，NV 无效时直接保底运行。
    dtu_log_info("storage default ready");
    dtu_storage_set_current_mode(dtu_board_detect_mode()); // 当前模式只由拨码决定，不从 NV 恢复。
    dtu_log_info("storage dip mode ready: %s", dtu_storage_mode_name(dtu_storage_current_mode()));
    dtu_storage_set_reboot_pending(false); // 启动时清除上次运行留下的重启等待态。
    ...
    ret = uapi_nv_read(NV_ID_DTU_CFG, sizeof(*blob), &real_len, (uint8_t *)blob); // 读取 base NV：角色、串口、Modbus、功率和白名单数量。
    ...
    if (!dtu_storage_apply_base_blob(blob)) {
        dtu_storage_set_default(dtu_storage_runtime()); // base 内容异常时回退默认配置，避免半加载状态。
        return ERRCODE_FAIL;
    }
    dtu_storage_load_wl_shards(); // base 校验通过后再恢复白名单分片。
    dtu_log_info("storage nv config applied: wl_count=%u", dtu_storage_runtime_const()->wl_count);
    return ERRCODE_SUCC;
}

/* 当前模式完全由拨码决定；NV 只保存可配置业务参数和白名单分片。 */
errcode_t dtu_storage_commit(void)
{
    ...
    if (!dtu_storage_pack_base_blob(blob)) { // 先打包不含白名单数组的 base NV。
    ...
    ret = uapi_nv_write(NV_ID_DTU_CFG, (const uint8_t *)blob, sizeof(*blob)); // base 配置先写入，白名单分片随后写。
    ...
    for (uint8_t i = 0; i < shard_count; i++) {
        if (!dtu_storage_pack_wl_shard(shard, i)) { // 每 16 条白名单打包成一个 NV shard。
        ...
        ret = uapi_nv_write(g_dtu_wl_shard_keys[i], (const uint8_t *)shard, sizeof(*shard)); // 写入当前白名单分片。
...
/* 恢复默认配置会清空自定义配置和白名单，当前模式继续由拨码决定。 */
errcode_t dtu_storage_factory_reset(void)
{
    dtu_storage_set_default(dtu_storage_runtime()); // 恢复出厂先重建默认 runtime。
    dtu_storage_set_current_mode(dtu_board_detect_mode()); // 模式继续跟随拨码，不随 factory reset 固化。
    dtu_storage_set_reboot_pending(false); // 清除重启等待态，避免恢复后拒绝写入。
    return dtu_storage_commit(); // 默认配置立即持久化到 NV。
}
