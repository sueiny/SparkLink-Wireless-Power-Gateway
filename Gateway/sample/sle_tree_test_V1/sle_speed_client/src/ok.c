...
static uint8_t g_app_uart_send_buff[256] = { 0 };
static osal_semaphore g_app_uart_rx_sem = {0};
static volatile uint16_t g_app_uart_tx_len = 0;
    ...
    ssapc_find_property_result_t *property, errcode_t status)
{
    ...
    // if ((property->operate_indication & SSAP_OPERATE_INDICATION_BIT_WRITE) != 0) {
    //     g_write_handle = property->handle;
    //     osal_printk("[ssap client] save write handle=0x%x\r\n", g_write_handle);
    // }
...
static void dtu_uart_rx_callback(const void *buffer, uint16_t length, bool error)
{
    ...
    if (buffer == NULL || length == 0 || error) {
        osal_printk("[DTU UART] rx callback: invalid params or error\r\n");
        return;
    }

    irq_status = osal_irq_lock();
    remain = (uint16_t)(sizeof(g_app_uart_tx_buff) - g_app_uart_tx_len);
    copy_len = (length > remain) ? remain : length;
    if (copy_len == 0 ||
        memcpy_s(&g_app_uart_tx_buff[g_app_uart_tx_len], remain, (const uint8_t *)buffer, copy_len) != EOK) {
        osal_irq_restore(irq_status);
        osal_printk("[DTU UART] memcpy failed\r\n");
        return;
    }
    g_app_uart_tx_len += copy_len;
    osal_irq_restore(irq_status);

    osal_sem_up(&g_app_uart_rx_sem);
    osal_printk("[DTU UART] rx chunk=%u cached=%u\r\n", length, g_app_uart_tx_len);
}

static void *dtu_uart_transparent_task(const char *arg)
{
    unused(arg);
    ...
    while (1) {
        ...
        if (osal_sem_down(&g_app_uart_rx_sem) != OSAL_SUCCESS) {
            osal_msleep(1);
            continue;
        }

        /* 透明测试模式：等待短时间，把同一次 PC 发送拆开的 UART chunk 合并成一包。 */
        osal_msleep(5);

        irq_status = osal_irq_lock();
        send_len = g_app_uart_tx_len;
        if (send_len > sizeof(g_app_uart_send_buff)) {
            send_len = sizeof(g_app_uart_send_buff);
        }
        if (send_len > 0) {
            (void)memcpy_s(g_app_uart_send_buff, sizeof(g_app_uart_send_buff), g_app_uart_tx_buff, send_len);
            g_app_uart_tx_len = 0;
        }
        osal_irq_restore(irq_status);
        ...
        param.type = SSAP_PROPERTY_TYPE_VALUE;
        param.data_len = send_len;
        param.data = g_app_uart_send_buff;

        ret = ssapc_write_req(0, g_conn_id, &param);
        osal_printk("[DTU UART] transparent send len=%u ret=0x%x conn=%u handle=0x%x range=[%u,%u]\r\n",
            send_len, ret, g_conn_id, param.handle, g_find_service_result.start_hdl, g_find_service_result.end_hdl);
    }
...
void uart_test_init(void)
{
    ...
    task = osal_kthread_create((osal_kthread_handler)dtu_uart_transparent_task, NULL, "DtuCliUartTask", 0x1000);
    if (task == NULL) {
        osal_printk("[DTU UART] transparent task create failed\r\n");
        return;
    }
