...
def load_mqtt_config():
    config_path = Path(__file__).resolve().parents[2] / "config" / "gateway_config.json"
    cfg = json.loads(config_path.read_text(encoding="utf-8"))
    tk = cfg["thingskit"]
    client_id = tk.get("client_id", cfg.get("gateway", {}).get("gateway_id", ""))
    if tk.get("credential_mode", "access_token") == "mqtt_basic":
        basic = tk.get("mqtt_basic", {})
        return {
            "broker": tk["host"],
            "port": tk["port"],
            "client_id": basic.get("client_id", client_id),
...
def gen_meter(name):
    ...
    if parent:
        ...
        data["meter_role"] = 0  # 支表
    else:
        data["meter_role"] = 1  # 总表
...
def gen_env():
    return {"temperature": round(random.uniform(20, 35), 1), "humidity": round(random.uniform(40, 80), 1), "online": 1}


def gen_relay():
    return {"relay_state": random.choice([0, 1]), "control_mode": 0, "online": 1}
...
def run(rounds=5):
    ...
    client = mqtt.Client(client_id=mqtt_cfg["client_id"], protocol=mqtt.MQTTv311)
    ...
    for i in range(1, rounds + 1):
        ...
        # 网关属性
        network_type, network_ifname = random.choice([
            ("ethernet", "eth0"),
            ("wifi", "wlan0"),
        ])
        gw = {
