/*
 * Copyright (c) 2025 Shenzhen Waveconn Techology Co., Ltd.
 *
 ...
 *
 * Description: SLE 1 vs 1 Demo(Client). \n
 *
 * History: \n
 * 2025-08-24, Create file. \n
 */
#include "securec.h"
...
{
    if (data->data_len == strlen("LED_ON") &&
        memcmp(data->data, "LED_ON", strlen("LED_ON")) == 0) {
        example_turn_onoff_led(GPIO_02, GPIO_LEVEL_HIGH);
        osal_printk("[SLE Client] LED ON\r\n");
    } else if (data->data_len == strlen("LED_OFF") &&
               memcmp(data->data, "LED_OFF", strlen("LED_OFF")) == 0) {
        example_turn_onoff_led(GPIO_02, GPIO_LEVEL_LOW);
        osal_printk("[SLE Client] LED OFF\r\n");
    }
...
static void example_sle_seek_result_info_cbk(sle_seek_result_info_t *seek_result_data)
{
    if (seek_result_data == NULL) {
        osal_printk("[SLE Client] seek result seek_result_data is NULL\r\n");
        return;
    }

    if (memcmp((void *)seek_result_data->addr.addr, (void *)g_sle_expected_addr, SLE_ADDR_LEN) == 0) {
        osal_printk("[SLE Client] seek result find expected addr:%02x***%02x%02x\r\n", seek_result_data->addr.addr[0],
...
{
    osal_printk("[SLE Client] conn state changed conn_id:0x%x, addr:%02x***%02x%02x\r\n", conn_id, addr->addr[0],
          addr->addr[4], addr->addr[5]); /* 0 4 5: addr index */
    osal_printk("[SLE Client] conn state changed disc_reason:0x%x\r\n", disc_reason);
...
{
    osal_printk("[SLE Client] pair complete conn_id:0x%x, status:0x%x, addr:%02x***%02x%02x\r\n", conn_id, status,
...
static void example_sle_exchange_info_cbk(uint8_t client_id,
                                          ...
                                          errcode_t status)
{
    osal_printk("[SLE Client] pair complete client id:0x%x, status:0x%x\r\n", client_id, status);
    osal_printk("[SLE Client] exchange mtu, mtu size:0x%x, version:0x%x.\r\n", param->mtu_size, param->version);
    if (status == ERRCODE_SUCC) {
...
static void example_sle_find_structure_cbk(uint8_t client_id,
                                           ...
                                           errcode_t status)
{
    osal_printk("[SLE Client] find structure cbk client:0x%x conn_id:0x%x status:0x%x \r\n", client_id, conn_id, status);
    osal_printk("[SLE Client] find structure start_hdl:[0x%04x], end_hdl:[0x%04x], uuid len:0x%x\r\n", service->start_hdl,
          service->end_hdl, service->uuid.len);
    if (service->uuid.len == UUID_16BIT_LEN) {
        osal_printk("[SLE Client] structure uuid:[0x%02x][0x%02x]\r\n", service->uuid.uuid[14],
              service->uuid.uuid[15]); /* 14 15: uuid index */
    } else {
        for (uint8_t idx = 0; idx < UUID_128BIT_LEN; idx++) {
            osal_printk("[SLE Client] structure uuid[0x%x]:[0x%02x]\r\n", idx, service->uuid.uuid[idx]);
        }
    }
...
static void example_sle_find_structure_cmp_cbk(uint8_t client_id,
                                               ...
                                               errcode_t status)
{
    osal_printk("[SLE Client] find structure cmp cbk client id:0x%x conn_id:0x%x status:0x%x type:%d uuid len:0x%x \r\n",
          client_id, conn_id, status, structure_result->type, structure_result->uuid.len);
    if (structure_result->uuid.len == UUID_16BIT_LEN) {
        osal_printk("[SLE Client] find structure cmp cbk structure uuid:[0x%02x][0x%02x]\r\n",
              structure_result->uuid.uuid[14], structure_result->uuid.uuid[15]); /* 14 15: uuid index */
    } else {
        for (uint8_t idx = 0; idx < UUID_128BIT_LEN; idx++) {
            osal_printk("[SLE Client] find structure cmp cbk structure uuid[0x%x]:[0x%02x]\r\n", idx,
                                          ...
                                          errcode_t status)
{
    ...
    for (uint16_t idx = 0; idx < property->descriptors_count; idx++) {
        osal_printk("[SLE Client] find property cbk, descriptors type [0x%x]: 0x%02x.\r\n", idx,
              property->descriptors_type[idx]);
    }
    if (property->uuid.len == UUID_16BIT_LEN) {
        osal_printk("[SLE Client] find property cbk, uuid: 0x%02x 0x%02x.\r\n", property->uuid.uuid[14],
        ...
        for (uint16_t idx = 0; idx < UUID_128BIT_LEN; idx++) {
            osal_printk("[SLE Client] find property cbk, uuid [0x%x]: 0x%02x.\r\n", idx, property->uuid.uuid[idx]);
        }
    }
}
...
static void example_sle_write_cfm_cbk(uint8_t client_id,
                                      ...
                                      errcode_t status)
{
    osal_printk("[SLE Client] write cfm cbk client id:0x%x conn_id:0x%x status:0x%x.\r\n", client_id, conn_id, status);
    osal_printk("[SLE Client] write cfm cbk handle:0x%x, type:0x%x\r\n", write_result->handle, write_result->type);
}
...
static void example_sle_read_cfm_cbk(uint8_t client_id,
                                     ...
                                     errcode_t status)
{
    osal_printk("[SLE Client] read cfm cbk client id:0x%x conn id:0x%x status:0x%x\r\n", client_id, conn_id, status);
    osal_printk("[SLE Client] read cfm cbk handle:0x%x, type:0x%x, len:0x%x\r\n", read_data->handle, read_data->type,
          read_data->data_len);
    for (uint16_t idx = 0; idx < read_data->data_len; idx++) {
        osal_printk("[SLE Client] read cfm cbk[0x%x] 0x%02x\r\n", idx, read_data->data[idx]);
    }
}
...
static void example_sle_notification_cbk(uint8_t client_id,
                                         ...
                                         errcode_t status)
{
    osal_printk("[SLE Client] notification cbk client id:0x%x conn id:0x%x status:0x%x\r\n", client_id, conn_id, status);

    osal_printk("[SLE Client] notification cbk handle:0x%x, type:0x%x data len:0x%x \r\n", data->handle, data->type,
          data->data_len);

    for (uint16_t idx = 0; idx < data->data_len; idx++) {
        osal_printk("[SLE Client] notification cbk[0x%x] 0x%02x\r\n", idx, data->data[idx]);
    }
...
static void example_sle_indication_cbk(uint8_t client_id,
                                       ...
                                       errcode_t status)
{
    osal_printk("[SLE Client] indication cbk client id:0x%x conn id:0x%x status:0x%x\r\n", client_id, conn_id, status);

    osal_printk("[SLE Client] indication cbk handle:0x%x, type:0x%x data len:0x%x \r\n", data->handle, data->type,
          data->data_len);

    for (uint16_t idx = 0; idx < data->data_len; idx++) {
        osal_printk("[SLE Client] indication cbk[0x%x] 0x%02x\r\n", idx, data->data[idx]);
    }
}
...
static int example_sle_led_client_task(const char *arg)
{
    ...
    if (sle_announce_seek_register_callbacks(&g_seek_cbk) != ERRCODE_SUCC) {
        osal_printk("[SLE Client] sle announce seek register callbacks fail !\r\n");
        return -1;
    }

    if (sle_connection_register_callbacks(&g_connect_cbk) != ERRCODE_SUCC) {
        osal_printk("[SLE Client] sle connection register callbacks fail !\r\n");
        return -1;
    }

    if (ssapc_register_callbacks(&g_ssapc_cbk) != ERRCODE_SUCC) {
        osal_printk("[SLE Client] sle ssapc register callbacks !\r\n");
        return -1;
    }
