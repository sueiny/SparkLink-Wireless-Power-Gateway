 ...
 *
 * Description: SLE 1 vs 1 Demo(Server). \n
 *
 * History: \n
...
typedef enum {
    ...
    EXAMPLE_CONTORL_LED_LEDBOARD_LED_ON = 0x03, //LED亮
    EXAMPLE_CONTORL_LED_LEDBOARD_LED_OFF = 0x04, //LED灭
...
{
    unused(arg);
    example_control_led_type_t last_led_operation = EXAMPLE_CONTORL_LED_LEDBOARD_LED_OFF;
    osal_printk("[SLE Server] start led control task\r\n");

  while (1)
  {
    (void)osal_msleep(1000); 

    if (last_led_operation == EXAMPLE_CONTORL_LED_LEDBOARD_LED_OFF) {
        uint8_t on[] = "LED_ON";
        example_sle_server_send_notify_by_handle(on, sizeof(on) - 1);//向client发送LED_ON
        last_led_operation = EXAMPLE_CONTORL_LED_LEDBOARD_LED_ON;
    } else {
        uint8_t off[] = "LED_OFF";
        example_sle_server_send_notify_by_handle(off, sizeof(off) - 1);//向client发送LED_OFF
        last_led_operation = EXAMPLE_CONTORL_LED_LEDBOARD_LED_OFF;
    }
  }
    return 0;
}
...
static void example_print_led_state(ssaps_req_write_cb_t *write_cb_para)
{
    if (write_cb_para->length == strlen("LED_ON") && write_cb_para->value[0] == 'L' && write_cb_para->value[1] == 'E' &&
        ...
        osal_printk("[SLE Server] client main board led is on.\r\n");
    }
    ...
    if (write_cb_para->length == strlen("LED_OFF") && write_cb_para->value[0] == 'L' &&
        ...
        osal_printk("[SLE Server] client main board led is off.\r\n");
    }
}
...
static void example_ssaps_read_request_cbk(uint8_t server_id,
                                           ...
                                           errcode_t status)
{
    osal_printk("[SLE Server] ssaps read request cbk server_id:0x%x, conn_id:0x%x, handle:0x%x, type:0x%x, status:0x%x\r\n",
...
static void example_ssaps_write_request_cbk(uint8_t server_id,
                                            ...
                                            errcode_t status)
{
    osal_printk("[SLE Server] ssaps write request cbk server_id:0x%x, conn_id:0x%x, handle:0x%x, status:0x%x\r\n", server_id,
          conn_id, write_cb_para->handle, status);

    for (uint16_t idx = 0; idx < write_cb_para->length; idx++) {
        osal_printk("[SLE Server] write request cbk[0x%x] 0x%02x\r\n", idx, write_cb_para->value[idx]);
    }
...
static void example_ssaps_mtu_changed_cbk(uint8_t server_id,
                                          ...
                                          errcode_t status)
{
    osal_printk("[SLE Server] ssaps mtu changed cbk server_id:0x%x, conn_id:0x%x, mtu_size:0x%x, status:0x%x\r\n", server_id,
...
{
    osal_printk("[SLE Server] start service cbk server_id:0x%x, handle:0x%x, status:0x%x\r\n", server_id, handle, status);
}
...
static errcode_t example_sle_server_service_add(void)
{
    ...
    if (ret != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle uuid add service fail, ret:0x%x\r\n", ret);
        return ERRCODE_FAIL;
    }

    osal_printk("[SLE Server] sle uuid add service service_handle: %u\r\n", g_service_handle);
...
static errcode_t example_sle_server_property_add(void)
{
    ...
    if (property.value == NULL) {
        osal_printk("[SLE Server] sle property mem fail\r\n");
    ...
    if (memcpy_s(property.value, sizeof(g_sle_property_value), g_sle_property_value, sizeof(g_sle_property_value)) !=
        ...
        osal_printk("[SLE Server] sle property mem cpy fail\r\n");
        return ERRCODE_MEMCPY;
    }
    ret = ssaps_add_property_sync(g_server_id, g_service_handle, &property, &g_property_handle);
    if (ret != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle uuid add property fail, ret:0x%x\r\n", ret);
        osal_vfree(property.value);
        return ERRCODE_FAIL;
    }

    osal_printk("[SLE Server] sle uuid add property property_handle: %u\r\n", g_property_handle);
    ...
    if (descriptor.value == NULL) {
        osal_printk("[SLE Server] sle descriptor mem fail\r\n");
        osal_vfree(property.value);
        return ERRCODE_MALLOC;
    }
    if (memcpy_s(descriptor.value, sizeof(ntf_value), ntf_value, sizeof(ntf_value)) != EOK) {
        osal_printk("[SLE Server] sle descriptor mem cpy fail\r\n");
        osal_vfree(property.value);
    ...
    if (ret != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle uuid add descriptor fail, ret:0x%x\r\n", ret);
        osal_vfree(property.value);
        osal_vfree(descriptor.value);
        return ERRCODE_FAIL;
    }
...
static errcode_t example_sle_server_add(void)
{
    ...
    osal_printk("[SLE Server] sle uuid add service in\r\n");
    ...
    osal_printk("[SLE Server] sle uuid add service, server_id:0x%x, service_handle:0x%x, property_handle:0x%x\r\n",
    ...
    osal_printk("[SLE Server] sle uuid add service out\r\n");
    return ERRCODE_SUCC;
}
...
static errcode_t example_sle_server_send_notify_by_handle(const uint8_t *data, uint8_t len)
{
    ...
    param.value_len = len;
    if (param.value == NULL) {
        osal_printk("[SLE Server] send notify mem fail\r\n");
        return ERRCODE_MALLOC;
    }

    if (memcpy_s(param.value, param.value_len, data, len) != EOK) {
        osal_printk("[SLE Server] send notify memcpy fail\r\n");
        osal_vfree(param.value);
        return ERRCODE_MEMCPY;
    }

    if (ssaps_notify_indicate(g_server_id, g_conn_id, &param) != ERRCODE_SUCC) {
        osal_printk("[SLE Server] ssaps notify indicate fail\r\n");
        osal_vfree(param.value);
        return ERRCODE_FAIL;
    }
    osal_vfree(param.value);
                                                  ...
                                                  sle_disc_reason_t disc_reason)
{
    ...
    osal_printk("[SLE Server] connect state changed addr:%02x:**:**:**:%02x:%02x\r\n", addr->addr[0], addr->addr[4],
          addr->addr[5]);
    g_conn_id = conn_id;
}
...
{
    osal_printk("[SLE Server] pair complete conn_id:0x%02x, status:0x%x\r\n", conn_id, status);
    osal_printk("[SLE Server] pair complete addr:%02x:**:**:**:%02x:%02x\r\n", addr->addr[0], addr->addr[4], addr->addr[5]);
...
static int example_sle_led_server_task(const char *arg)
{
    ...
    osal_printk("[SLE Server] try enable.\r\n");
    /* 使能SLE */
    ...
    if (example_sle_conn_register_cbks() != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle conn register cbks fail !\r\n");
        return -1;
    }

    /* 注册 SSAP server 回调函数 */
    if (example_sle_ssaps_register_cbks() != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle ssaps register cbks fail !\r\n");
        return -1;
    }

    /* 注册Server, 添加Service和property, 启动Service */
    if (example_sle_server_add() != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle server add fail !\r\n");
        return -1;
    }

    /* 设置设备公开，并公开设备 */
    if (example_sle_server_adv_init() != ERRCODE_SUCC) {
        osal_printk("[SLE Server] sle server adv fail !\r\n");
        return -1;
    }

    osal_printk("[SLE Server] init ok\r\n");
