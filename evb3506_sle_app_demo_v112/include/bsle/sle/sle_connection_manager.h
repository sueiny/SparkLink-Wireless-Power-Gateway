...
typedef enum {
    SLE_DISCONNECT_UNKNOWN           = 0x00,    /*!< @if Eng unknown disconnect reason
                                                     @else   未知原因断链 @endif */
    SLE_ERR_PIN_OR_KEY_MISSING       = 0x05,    /*!< @if Eng pin or key missing
                                                     @else   pin或key丢失 @endif */
    SLE_ERR_CONN_TIMEOUT             = 0x07,    /*!< @if Eng disconnect by timeout
                                                     @else   连接超时断链 @endif */
    SLE_DISCONNECT_BY_REMOTE         = 0x10,    /*!< @if Eng disconnect by remote
                                                     @else   远端断链 @endif */
    SLE_DISCONNECT_BY_LOCAL          = 0x11,    /*!< @if Eng disconnect by local
                                                     @else   本端断链 @endif */
    SLE_CONN_TERMINATE_BY_MIC_ERROR  = 0x1B,    /*!< @if Eng connection terminated due to mic failure
                                                     @else   mic error断连 @endif */
    SLE_CONN_FAIL_TO_ESTABLISH       = 0x1C,    /*!< @if Eng connection failed to be established
                                                     @else   建连异常 @endif */
} sle_disc_reason_t;
