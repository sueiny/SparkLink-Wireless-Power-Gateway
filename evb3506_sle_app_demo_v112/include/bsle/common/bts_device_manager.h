...
/**
 * @if Eng
 * @brief Callback invoked to send device dfr event to app.
 * @par device dfr event.
 * @attention 1.This function is called in dfr context, should not be blocked or do long time waiting.
 * @param [in] event        device dfr event.
 * @retval no return value.
 * @par Dependency:
 * @li  bts_def.h
 * @see bts_dev_manager_callbacks_t
 * @else
 * @brief  ble 设备 dfr 事件通知回调函数。
 * @par    设备dfr 事件。
 * @attention  1. 该回调函数运行于dfr流程，不能阻塞或长时间等待。
 * @param [in] event        设备dfr事件.
 * @retval 无返回值。
 * @par 依赖:
 * @li  bts_def.h
 * @see bts_dev_manager_callbacks_t
 * @endif
 */
typedef void (*bts_ble_dfr_callback)(uint8_t event);
...
typedef struct {
    ...
    bts_ble_dfr_callback ble_dfr_cb;                          /*!< @if Eng dfr callback
                                                                   @else   dfr回调函数 @endif */
