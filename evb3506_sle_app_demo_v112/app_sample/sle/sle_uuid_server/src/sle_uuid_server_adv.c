...
/* 广播名称 */
static uint8_t sle_local_name[] = "sle_uuid_server";
...
#define SLE_ADV_TIMEOUT 20 // timeout 时间是 20 * 10ms = 200 ms

/* sle adv  enable status */
static uint8_t  g_sle_adv_status = 0;
...
static uint16_t sle_set_adv_data(uint8_t *adv_data)
{
    ...
    uint8_t hid_uuid[] = {0x0B, 0x06};
    uint8_t uuid_len = sizeof(hid_uuid);
    adv_data[idx++] = SLE_ADV_DATA_TYPE_COMPLETE_LIST_OF_16BIT_SERVICE_UUIDS;
    adv_data[idx++] = uuid_len;
    if (memcpy_s(&adv_data[idx], SLE_ADV_DATA_LEN_MAX - idx, &hid_uuid, uuid_len) != EOK) {
        sample_at_log_print("%s adv 16bit uuid memcpy fail\r\n", SLE_UUID_SERVER_LOG);
        return 0;
    }
    idx += uuid_len;
...
errcode_t sle_uuid_announce_register_cbks(void)
{
    ...
    seek_cbks.sle_disable_cb = sle_disable_cbk;
    ret = sle_announce_seek_register_callbacks(&seek_cbks);
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_uuid_announce_register_cbks,register_callbacks fail :0x%x\r\n",
...
errcode_t sle_uuid_server_adv_init(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        sample_at_log_print("%s sle_uuid_server_adv_init,sle_start_announce fail :0x%x\r\n", SLE_UUID_SERVER_LOG, ret);
        return ret;
    }
    sle_wait_result_handle(SLE_ADV_TIMEOUT, &g_sle_adv_status, 1);
    return ERRCODE_SLE_SUCCESS;
}
...
errcode_t sle_uuid_server_adv_deinit(void)
{
    ...
    sle_wait_result_handle(SLE_ADV_TIMEOUT, &g_sle_adv_status, 0);
    return ERRCODE_SLE_SUCCESS;
}
